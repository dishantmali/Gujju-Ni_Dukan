                            ...
                            {directoryProductsList.map(p => {
                              ...
                              return (
                                <tr key={p.id} className="hover:bg-[var(--bg-main)] transition-colors duration-150">
                                  ...
                                  <td className="px-6 py-4"><span className="text-xs font-bold text-[var(--brown-mid)] uppercase tracking-widest bg-[var(--brown-mid)]/10 px-2 py-1 rounded-md">{p.vendor_shop}</span></td>
                                  <td className="px-6 py-4 font-bold text-[var(--text-dark)]">₹{parseFloat(p.price).toLocaleString()}</td>
                                  <td className="px-6 py-4 text-center">{pIsActive ? <span className="px-3 py-1 bg-green-100 text-green-700 text-[10px] font-black uppercase tracking-wider rounded-md">Enabled</span> : <span className="px-3 py-1 bg-red-100 text-red-700 text-[10px] font-black uppercase tracking-wider rounded-md">Disabled</span>}</td>
                                  <td className="px-6 py-4"><div className="flex justify-end gap-3">
                                    ...
                                    <button onClick={() => handleAction('product', p.id, pIsActive ? 'reject' : 'approve')} className={`px-5 py-2 text-xs font-bold uppercase rounded-lg transition-colors border ${pIsActive ? 'text-red-600 border-red-200 hover:bg-red-50' : 'text-[var(--text-muted)] border-[var(--border)] hover:bg-[var(--bg-secondary)]'}`}>{pIsActive ? 'Disable' : 'Enable'}</button>
                                  </div></td>
                                </tr>
                              )
                            })}
